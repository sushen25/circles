...
describe('handOffRefusal', () => {
  it('lets a saved-place member take it over', () => {
    expect(handOffRefusal({ userId: PRIYA, isMember: true, isPermanent: true }, MAYA)).toBe(
      undefined,
    );
  });

  it('refuses a guest, somebody who has left, and the organiser themselves', () => {
    expect(handOffRefusal({ userId: PRIYA, isMember: true, isPermanent: false }, MAYA)).toBe(
      'requires_saved_place',
    );
    expect(handOffRefusal({ userId: PRIYA, isMember: false, isPermanent: true }, MAYA)).toBe(
      'not_a_member',
    );
    expect(handOffRefusal({ userId: MAYA, isMember: true, isPermanent: true }, MAYA)).toBe(

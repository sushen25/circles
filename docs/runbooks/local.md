...
| `pnpm test:e2e:live` | yes | Exports the web app pointed at the local stack and walks the journeys in five browser projects: mobile Safari and Messenger's iOS browser (WebKit), Chrome and WhatsApp's Android browser (Chromium), and mobile Safari again in `en-AU` against the `en-US` export (ADR 00XX). Needs `pnpm exec playwright install chromium webkit` once. |

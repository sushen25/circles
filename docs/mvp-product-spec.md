...
4. The threshold is `min(n, max(3, ceil(n / 4)))` for `n` active members when the ask is made — three for circles of up to twelve, a quarter of the circle above that, five at twenty ([ADR 00XX](decisions/00XX-the-quiet-ask-at-twenty-members.md)). It is fixed when the ask is made and is not editable in MVP.

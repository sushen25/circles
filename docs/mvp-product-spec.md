...
- One at a time: a circle has at most one plan finding a time (`collecting` or `ready`). While one is, **Plan a catch-up** shows that plan — with **Edit** for its organiser and **Cancel** for the organiser or the owner — instead of the setup form; `create-plan` refuses a second (`plan_in_progress`), and so do **Change the time** on a locked-in plan and a quiet ask reaching its threshold ([ADR 00XX](decisions/00XX-one-open-plan-per-circle.md)).

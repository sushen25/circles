...
| `join-plan` | anonymous or permanent | Verify Turnstile (web); given a plan short code and, for a guest, a display name: if the plan is taking answers (`collecting` or `ready`, deadline ahead), create the membership under the same cap and duplicate-name rules as `redeem-invite` **and** add the caller to the plan's current participants, in one transaction. For an existing member, only the participant row. Every refusal to admit is the one `invite_inactive`, so a quiet ask's code reads like an unknown one. Rate-limited per code and per address ([ADR 0022](decisions/0022-a-plan-link-admits-new-members-while-the-plan-is-asking.md)) |

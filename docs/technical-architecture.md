...
| `request-email-updates` | member | Normalise, dedupe per identity, create the contact and record the consent as given ([ADR 0019](decisions/0019-consent-is-recorded-when-it-is-given.md)), issue the verification token, enqueue the verification email. Answers identically for a new, verified, shared or suppressed address |

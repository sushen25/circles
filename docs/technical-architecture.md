...
| Table | Columns of note | Constraints |
|---|---|---|
| `meetup_confirmations` | `plan_id`, `revision`, `candidate_id`, `starts_at`, `ends_at`, `place_name`, `place_url`, `note`, `confirmed_by`, `status` (`active|superseded|cancelled|completed`), `superseded_at`, `cancel_reason` | one `active` per `(plan_id, revision)` (partial unique) |
| `attendance` | `confirmation_id`, `user_id`, `status` (`going|cant|unknown|was_there|missed`), `updated_at` | unique `(confirmation_id, user_id)` |
| `outcome_reports` | `confirmation_id`, `reported_by`, `outcome` (`happened|cancelled|moved_outside|not_sure`), `note`, `reported_at` | one per confirmation per reporter |

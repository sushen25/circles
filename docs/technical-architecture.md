...
| `get-invite-link` | owner | The live invite's secret, re-derived and checked against the stored digest, or `null` when it cannot be shown ([ADR 00XX](decisions/00XX-an-invite-secret-is-derived-so-its-owner-can-see-it-again.md)); a read, no idempotency record |

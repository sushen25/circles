...
| `answer-interest` | member | Records interest in `private`; atomically evaluates threshold under a plan row lock; transitions once. An ask held at its threshold beside an open plan is tried again on every answer and on every dispatcher sweep ([ADR 0035](decisions/0035-the-quiet-ask-at-twenty-members.md)). Answers `{ threshold_reached }` and never a count |

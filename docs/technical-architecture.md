...
| `join-plan` | anonymous or permanent | Verify Turnstile (web); given a plan short code and a display name (required for a guest; optional for an account, which otherwise joins under its profile name, and sends one after a `duplicate_name`): if the plan is taking answers (`collecting` or `ready`, deadline ahead), create the membership under the same cap and duplicate-name rules as `redeem-invite` **and** add the caller to the plan's current participants, in one transaction. For an existing member, only the participant row. Every refusal to admit is the one `invite_inactive`, so a refusal does not distinguish an unknown code from a quiet ask, a confirmed plan or a cancelled one. (A join that succeeds does say the plan is taking answers; that is not hidden.) Rate-limited per code and per address ([ADR 0022](decisions/0022-a-plan-link-admits-new-members-while-the-plan-is-asking.md)) |

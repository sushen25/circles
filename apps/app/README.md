...
**No longer enough since S3-01a.** Answer drafts are kept in
`react-native-mmkv`, a Nitro module Expo Go does not bundle, and the drafts
module is loaded with the app, so expect the app not to start in Expo Go. Use a
...
this repository can fix it. **Still true on 26 September 2026** (S3-01a): Expo
SDK 57.0.20, `expo-modules-core` 57.0.16 (now precompiled) and
`expo-modules-jsi` 57.0.8 fail on the same two lines; 57.1.1, the newest for
SDK 57, still carries both annotations, and `expo-modules-core` pins `~57.0.8`. Two ways out:
...
### Against the local stack, and the native smoke

`make native-android`, `make native-metro` and `make test-native` build it,
serve it and run the Maestro smoke (`tests/native/`) against this checkout's
stack and ports; the debug build's Metro host and `adb reverse` are what make
the emulator reach them. The details are in `docs/runbooks/local.md` §8.

Two native modules arrived with S3-01a and need a rebuild, not just a reload:
`react-native-mmkv` (with `react-native-nitro-modules`), for answer drafts, and
`expo-crypto`, for the invite digest and idempotency keys (Hermes has no Web
Crypto).

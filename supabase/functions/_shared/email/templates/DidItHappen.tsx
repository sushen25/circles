...
import { attendanceLink, outcomeLink, settingsLink, subscriberFooter } from '../links.ts';
 ...
 * to somebody who attended reads as though nobody told them — and two buttons,
 * each opening the screen that asks its own question (S1-29).
 */
export function DidItHappen({ input }: { input: DidItHappenInput }): ReactNode {
  return (
    <Layout
      copy={didItHappenCopy(input)}
      buttonUrl={outcomeLink(input.origin, input.planCode)}
      footer={{
        kind: 'reason',
...
export function DidItHappenParticipant({
  ...
  return (
    <Layout
      ...
      buttonUrl={attendanceLink(input.origin, input.planCode)}
